...
def _parse_feed(xml_text: str, channel_id: str) -> list[dict]:
    """解析 RSS Feed XML，提取视频信息列表。"""
    root = ET.fromstring(xml_text)
    videos: list[dict] = []
    feed_author = root.find("atom:author/atom:name", _NS)
    ...
    for entry in root.findall("atom:entry", _NS):
        ...
        ch_id_el = entry.find("yt:channelId", _NS)
        ch_id = ch_id_el.text if ch_id_el is not None and ch_id_el.text else channel_id
        author_name_el = entry.find("atom:author/atom:name", _NS)
        ch_name = author_name_el.text if author_name_el is not None and author_name_el.text else feed_channel_name
        thumb_el = entry.find("media:group/media:thumbnail", _NS)
        thumbnail_url = thumb_el.get("url") if thumb_el is not None and thumb_el.get("url") else f"https://i.ytimg.com/vi/{video_id}/mqdefault.jpg"
        videos.append({"video_id": video_id, "thumbnail_url": thumbnail_url, "channel_id": ch_id, "channel_name": ch_name})
    return videos
...
def run_workflow(task_id: str, payload: dict) -> None:
    ...
    for ch_id in channel_ids:
        try:
            xml_text = _fetch_feed(ch_id)
            videos = _parse_feed(xml_text, ch_id)
            all_videos.extend(videos)
            success_ch += 1
            write_task_log(task_id, "channel_fetch", ch_id, "success", f"获取到 {len(videos)} 个视频")
        except Exception as exc:
            fail_ch += 1
            write_task_log(task_id, "channel_fetch", ch_id, "failed", str(exc))
    ...
    for video in all_videos:
        ...
        try:
            ...
            write_task_log(task_id, "image_download", image_name, "failed", str(exc))
...
def run_retry_workflow(task_id: str, payload: dict) -> None:
    ...
    run_mode = payload.get("run_mode", "batch")
    if run_mode == "batch":
        stats = _run_batch_inference(task_id, failed_images, payload)
    else:
        stats = _run_realtime_inference(task_id, failed_images, payload)
...
def _poll_batch_job(task_id: str, batch_job_arn: str, bedrock_client, interval: int = 60) -> str:
    ...
    while True:
        ...
        if raw_status in ("InProgress", "Validating", "Scheduled"):
...
def _run_realtime_inference(task_id: str, images: list[dict], payload: dict) -> dict:
    ...
    write_task_log(task_id, "model_invoke", "realtime",
                   "success" if failed_count == 0 else "failed",
                   f"实时推理完成: 成功={success_count}, 失败={failed_count}")
...
async def _invoke_single_rt(
        ...
        for attempt in range(max_retries):
            try:
                ...
                write_task_log(task_id, "model_invoke", image_name, "success",
                               f"实时推理成功: review_result={review_result}")
    ...
    put_item(results_table, {
        ...
        "video_id": image.get("video_id", ""), "channel_id": image.get("channel_id", ""),
        "channel_name": image.get("channel_name", ""), "s3_key": s3_key,
        "status": "failed", "result_json": {}, "review_result": "",
